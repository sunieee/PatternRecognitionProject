...
```bash
sudo docker pull Object Detection_yolo/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it Object Detection_yolo/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it -v "$(pwd)"/datasets:/usr/src/datasets Object Detection_yolo/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it --gpus all Object Detection_yolo/yolov5:latest
```
